...
import torch
from .network.AttUnet_model import Att_UNet
# device
...
def predict_evaluation(model, test_patient_path, structure_list):
    ...
    s_n = len(structure_list)  # s_n is the number of structures
    X =  batch_XY[:,:,:,:,0:s_n]
    ...
    metrics = _evaluate(plan, plan_hat, structure_list=HN_structures)
    return metrics
...
def predict_batch(model_path, test_path,  structure_list):
    ...
    for folder in data_dirs:
        if os.path.isdir(data_folder+folder):
            ...
            metrics = predict_evaluation(model, test_patient_path, structure_list= structure_list)
            metrics_all[folder] = metrics

    return metrics_all

def metrics_summary(metrics_all, structure_list):
    pts = list(metrics_all.keys())
    df = metrics_all[pts[0]].copy()
  #  organs = list(df.index.values)
    organs = structure_list
...
def predict_unit_test(test_path = 'Data/HN_data/npy_dataset/test/'):
    ''' test function run in jupyter
    '''
    print(test_path)
    model_path = str(Path.cwd())+'/models/HN/64x64/best_Unet_model.pt'
    metrics_all = predict_batch(model_path, test_path,  structure_list=HN_structures)
    df_mean, df_std  = metrics_summary(metrics_all, structure_list = HN_structures)
    return df_mean, df_std
