...
### Below are the test functions for used in Plan class, they are not used in plan generation
...
def plot_DVH(plan, structure_list):
    ...
    for s in structure_list:
        ...
        DVH_all[s] = DVH
        print(s)
        print('mean organ dose is: ', dose_organ.mean())
        print('max organ dose is: ', dose_organ.max())
    fig = plt.figure()
    for s in structure_list:
        r = random.uniform(0, 1); g = random.uniform(0, 1); b = random.uniform(0, 1)
        plt.plot(dose_bin,DVH_all[s]*100, color = (r,g,b), linewidth=1)
